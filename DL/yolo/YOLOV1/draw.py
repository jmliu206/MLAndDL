...
_RED = (20,50,255)
_WHITE = (255, 255, 255)
def vis_class(img, pos, class_str="person", font_scale=0.35,label=1):
    ...
    back_br = x0 + txt_w, y0
    cv2.rectangle(img, back_tl, back_br, temp_GREEN, -1) # _GREEN
    # Show text.
    txt_tl = x0, y0 - int(0.2 * txt_h)
    cv2.putText(img, txt, txt_tl, font, font_scale, _GRAY, lineType=cv2.LINE_AA)
    cv2.rectangle(img,(pos[0],pos[1]),(pos[2],pos[3]),temp_GREEN,2) # _GREEN
    return img
...
# 画框
def draw_rect(img,target):
    boxes, labels=target["boxes"],target["labels"]
    img=np.asarray(img,np.uint8).copy()
    for pos,label in zip(boxes,labels):
        class_str = "%s" % (label)
        img=vis_class(img,list(map(int,pos)),class_str,0.5,label)
